...
app = App(title="Carica grafici", width=800, bg="light blue", height=400, layout="grid")
button1 = PushButton(app, command=carica_file_csv,  text="clicca qui per caricare un file csv", grid=[17,0])
button2 = PushButton(app, command=getGraphRoot,  text="clicca qui per caricare un grafico", grid=[18,0])
button3 = PushButton(app, command=genera_grafico,  text="clicca qui per generare un grafico", grid=[19,0])
button4 = PushButton(app, command=app.destroy, text="Clicca qui per chiudere la scheda", grid=[20,0])
app.display()